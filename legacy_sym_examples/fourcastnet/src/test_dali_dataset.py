...
def _to_cpu(batch):
    return {
        k: (v.detach().to("cpu") if isinstance(v, torch.Tensor) else torch.as_tensor(v))
...
def test_dali_dataset_basic(
    ...
    for _ in range(num_epochs):
        ...
        for batch_base, batch_dali in zip(base_loader, dali_loader):
            ...
            invar_d, outvar_d, lw_d = (_to_cpu(i) for i in batch_dali)
...
def test_distributed_dali_loader(data_path: Path):
    ...
    idx_start = num_samples_per_rank * rank
