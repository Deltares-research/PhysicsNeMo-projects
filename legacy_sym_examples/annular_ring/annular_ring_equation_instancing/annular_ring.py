...
def split_residuals(model: FlowNet, x: torch.Tensor, y: torch.Tensor, nu: float):
    ...
    # Equation instancing style split: pressure-convective part and viscous part.
    momentum_x_0 = u * u_x + v * u_y + p_x
    momentum_y_0 = u * v_x + v * v_y + p_y
    momentum_x_1 = -nu * (u_xx + u_yy)
    momentum_y_1 = -nu * (v_xx + v_yy)

    return continuity, momentum_x_0, momentum_y_0, momentum_x_1, momentum_y_1
...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    for step in range(int(cfg.training.max_steps)):
        ...
        c0, mx0, my0, mx1, my1 = split_residuals(model, x_i, y_i, nu)
        z = torch.zeros_like(c0)
        loss_pde = mse(c0, z) + mse(mx0 + mx1, z) + mse(my0 + my1, z)

        u_flux, _, _ = model(x_out, y_out)
        loss_flux = mse(2.0 * torch.mean(u_flux), torch.tensor(2.0, device=device))
