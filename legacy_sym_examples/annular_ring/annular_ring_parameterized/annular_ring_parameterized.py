...
class FlowNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, r: torch.Tensor):
        out = self.net(torch.cat([x, y, r], dim=1))
...
def residuals(model: FlowNet, x: torch.Tensor, y: torch.Tensor, r: torch.Tensor, nu: float):
...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    r_min = float(cfg.geometry.inner_radius_min)
    r_max = float(cfg.geometry.inner_radius_max)
    ...
    for step in range(int(cfg.training.max_steps)):
        ...
        r_cur = float(np.random.uniform(r_min, r_max))
        r_in = torch.full((int(cfg.batch_size.inlet), 1), r_cur, device=device)
        y_in = torch.empty(int(cfg.batch_size.inlet), 1, device=device).uniform_(-1.0, 1.0)
        x_in = torch.full_like(y_in, x_bounds[0])
        u_in, v_in, _ = model(x_in, y_in, r_in)
        loss_in = mse(u_in, inlet_profile(y_in, inlet_vel)) + mse(v_in, torch.zeros_like(v_in))

        r_out = torch.full((int(cfg.batch_size.outlet), 1), r_cur, device=device)
        y_out = torch.empty(int(cfg.batch_size.outlet), 1, device=device).uniform_(-1.0, 1.0)
        x_out = torch.full_like(y_out, x_bounds[1])
        _, _, p_out = model(x_out, y_out, r_out)
        loss_out = mse(p_out, torch.zeros_like(p_out))

        x_ns, y_ns = sample_no_slip(int(cfg.batch_size.no_slip), x_bounds, r_outer, r_cur, device)
        r_ns = torch.full((x_ns.shape[0], 1), r_cur, device=device)
        u_ns, v_ns, _ = model(x_ns, y_ns, r_ns)
        loss_ns = mse(u_ns, torch.zeros_like(u_ns)) + mse(v_ns, torch.zeros_like(v_ns))

        x_i, y_i = sample_interior(int(cfg.batch_size.interior), x_bounds, y_bounds, r_outer, r_cur, device)
        r_i = torch.full((x_i.shape[0], 1), r_cur, device=device)
        c, mx, my = residuals(model, x_i, y_i, r_i, nu)
        z = torch.zeros_like(c)
        loss_pde = mse(c, z) + mse(mx, z) + mse(my, z)

        flux = 2.0 * torch.mean(model(x_out, y_out, r_out)[0])
        ...
        if step % int(cfg.training.log_every) == 0:
            print(
                f"step={step:06d} r={r_cur:.4f} total={loss.item():.4e} inlet={loss_in.item():.4e} "
                f"outlet={loss_out.item():.4e} no_slip={loss_ns.item():.4e} interior={loss_pde.item():.4e} "
