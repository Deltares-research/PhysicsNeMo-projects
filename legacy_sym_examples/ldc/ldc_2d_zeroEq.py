...
@hydra.main(version_base="1.3", config_path="conf_zeroEq", config_name="config")
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    nu_min = float(cfg.physics.nu_min)
    ...
    for step in range(int(cfg.training.max_steps)):
        ...
        loss_top = mse(w_top * u_top, w_top * (1.5 * torch.ones_like(u_top))) + mse(v_top, torch.zeros_like(v_top))
        ...
        v = int_out[:, 1:2]
        p = int_out[:, 2:3]
        nu_eff = nu_min + softplus(int_out[:, 3:4])

        continuity, mx, my, _ = ns_residual(u, v, p, interior, nu_eff)
        ...
        gnu = torch.autograd.grad(
            nu_eff,
            interior,
            grad_outputs=torch.ones_like(nu_eff),
            create_graph=True,
        )[0]
        loss_nu_smooth = mse(gnu[:, 0:1], torch.zeros_like(gnu[:, 0:1])) + mse(
            gnu[:, 1:2], torch.zeros_like(gnu[:, 1:2])
        )
        ...
        loss = (
            ...
            + float(cfg.loss.nu_smooth) * loss_nu_smooth
        )
        loss.backward()
        optimizer.step()
        scheduler.step()
        ...
        if step % int(cfg.training.log_every) == 0:
            msg = (
                ...
                f"nu_smooth={loss_nu_smooth.item():.4e}"
            )
            if val:
                with torch.no_grad():
                    xy = torch.cat([val["x"], val["y"]], dim=1)
                    pred = model(xy)
                    nu_pred = nu_min + softplus(pred[:, 3:4])
                    val_mse = mse(pred[:, 0:1], val["u"]) + mse(pred[:, 1:2], val["v"])
                    if "nuT" in val:
                        val_mse += mse(nu_pred, val["nuT"] + nu_min)
