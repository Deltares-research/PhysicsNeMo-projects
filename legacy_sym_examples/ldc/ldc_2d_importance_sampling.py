...
def sample_importance_points(
    ...
    pool = sample_interior(n_pool, width, height, device).requires_grad_(True)
    out = model(pool)
    u = out[:, 0:1]
    v = out[:, 1:2]
    gu = torch.autograd.grad(u, pool, grad_outputs=torch.ones_like(u), create_graph=True)[0]
    gv = torch.autograd.grad(v, pool, grad_outputs=torch.ones_like(v), create_graph=True)[0]
    imp = torch.sqrt(gu[:, 0] ** 2 + gu[:, 1] ** 2 + gv[:, 0] ** 2 + gv[:, 1] ** 2) + 10.0
    prob = (imp / torch.sum(imp)).detach()
    idx = torch.multinomial(prob, n_pick, replacement=True)
    return pool[idx].detach().requires_grad_(True)
...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    for step in range(int(cfg.training.max_steps)):
        ...
        interior = sample_importance_points(
            model,
            int(cfg.batch_size.Interior),
            int(cfg.importance.pool_size),
            width,
            height,
            device,
        )
