...
def dict_to_csv(data_dict, file_stem):
    keys = list(data_dict.keys())
    n_rows = data_dict[keys[0]].shape[0]
    with open(f"{file_stem}.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(keys)
        for i in range(n_rows):
            row = [float(np.asarray(data_dict[k]).reshape(-1)[i]) for k in keys]
            writer.writerow(row)
