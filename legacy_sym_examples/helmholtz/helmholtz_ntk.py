...
def grad_norm(loss: torch.Tensor, model: nn.Module) -> torch.Tensor:
    ...
    return torch.sqrt(sq_sum + 1.0e-12)


@hydra.main(version_base="1.3", config_path="conf", config_name="config_ntk")
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    wall_w = float(cfg.loss.wall)
    interior_w = float(cfg.loss.interior)
    use_adaptive = bool(cfg.training.adaptive_weights.use)
    update_every = int(cfg.training.adaptive_weights.update_every)
    ...
    for step in range(int(cfg.training.max_steps)):
        ...
        if use_adaptive and step % update_every == 0:
            with torch.enable_grad():
                g_wall = grad_norm(loss_wall, model)
                g_pde = grad_norm(loss_pde, model)
                denom = (g_wall + g_pde).item()
                if denom > 0.0:
                    wall_w = float((g_pde / (g_wall + g_pde)).item())
                    interior_w = float((g_wall / (g_wall + g_pde)).item())

        loss = wall_w * loss_wall + interior_w * loss_pde
        loss.backward()
        optimizer.step()
        scheduler.step()
        ...
        if step % int(cfg.training.log_every) == 0:
            msg = (
                ...
                f"w_wall={wall_w:.3f} w_int={interior_w:.3f}"
            )
