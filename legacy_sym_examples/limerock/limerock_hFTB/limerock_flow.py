...
from limerock_properties import limerock, nd_fluid_density, nd_fluid_viscosity, nd_inlet_velocity, nd_volumetric_flow
...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    nu_val = float(nd_fluid_viscosity / nd_fluid_density)
    ...
    for step in range(int(cfg.training.max_steps)):
        ...
        loss_in = mse(u_i, torch.full_like(u_i, float(nd_inlet_velocity))) + mse(v_i, torch.zeros_like(v_i)) + mse(w_i, torch.zeros_like(w_i))
        ...
        loss_flux = mse(flux_est, torch.tensor(float(nd_volumetric_flow), device=device))
