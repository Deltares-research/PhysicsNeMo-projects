...
class TurbNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        om_plus = torch.nn.functional.softplus(out[:, 4:5])
        return u, v, p, k, om_plus
...
@hydra.main(version_base="1.3", config_path="conf_re590_k_om_LS", config_name="config")
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    for step in range(int(cfg.training.max_steps)):
        ...
        u_wf, v_wf, _, _, om_wf = model(x_wf, y_wf)
        loss_wf = mse(u_wf, torch.zeros_like(u_wf)) + mse(v_wf, torch.zeros_like(v_wf)) + mse(om_wf, torch.zeros_like(om_wf))
        ...
        u0, v0, p0, k0, om0 = model(x0, y0)
        loss_init = mse(u0, torch.zeros_like(u0)) + mse(v0, torch.zeros_like(v0)) + mse(p0, torch.zeros_like(p0)) + mse(k0, torch.zeros_like(k0)) + mse(om0, torch.zeros_like(om0))
