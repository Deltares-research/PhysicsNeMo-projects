...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    for step in range(int(cfg.training.max_steps)):
        ...
        wsel = torch.randint(0, 3, (xyz_wall.shape[0],), device=device)
        xyz_wall[wsel == 0, 1] = y_bounds[1]
        xyz_wall[wsel == 1, 2] = z_bounds[0]
        xyz_wall[wsel == 2, 2] = z_bounds[1]
        ...
        loss_sym_f = torch.tensor(0.0, device=device)
        if int(cfg.batch_size.get("symmetry_fluid", 0)) > 0:
            xyz_sf = sample_box(int(cfg.batch_size.symmetry_fluid), x_bounds, (0.0, 0.0), z_bounds, device)
            xyz_sf[:, 1:2] = 0.0
            xyz_sf = xyz_sf.requires_grad_(True)
            tsf = fluid_net(xyz_sf)
            gsf = grad(tsf, xyz_sf)
            loss_sym_f = mse(gsf[:, 1:2], torch.zeros_like(gsf[:, 1:2]))

        loss_sym_s = torch.tensor(0.0, device=device)
        if int(cfg.batch_size.get("symmetry_solid", 0)) > 0:
            xyz_ss = sample_box(int(cfg.batch_size.symmetry_solid), chip_x, (0.0, 0.0), chip_z, device)
            xyz_ss[:, 1:2] = 0.0
            xyz_ss = xyz_ss.requires_grad_(True)
            tss = solid_net(xyz_ss)
            gss = grad(tss, xyz_ss)
            loss_sym_s = mse(gss[:, 1:2], torch.zeros_like(gss[:, 1:2]))
        ...
        loss = (
            ...
            + float(cfg.loss.symmetry_fluid) * loss_sym_f
            + float(cfg.loss.symmetry_solid) * loss_sym_s
        )
