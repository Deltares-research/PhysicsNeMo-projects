...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    for step in range(int(cfg.training.max_steps)):
        ...
        wall_sel = torch.randint(0, 3, (xyz_wall.shape[0],), device=device)
        xyz_wall[wall_sel == 0, 1] = y_bounds[1]
        xyz_wall[wall_sel == 1, 2] = z_bounds[0]
        xyz_wall[wall_sel == 2, 2] = z_bounds[1]
        ...
        xyz_sym = sample_box(int(cfg.batch_size.symmetry), x_bounds, (0.0, 0.0), z_bounds, device)
        xyz_sym[:, 1:2] = 0.0
        _, v_sym, _, _ = model(xyz_sym)
        loss_sym = mse(v_sym, torch.zeros_like(v_sym))
        ...
        loss = (
            ...
            + float(cfg.loss.symmetry) * loss_sym
        )
